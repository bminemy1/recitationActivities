...
\begin{problem}
Sketch a graph of a function that is continuous on $(\-infty,\infty) has has the following properties.
	\begin{enumerate}
	\item Function $f$ does not have a local maximum or minimum.  $f$ contains a point where $f'(x)=0$
	
	
	
	\item Function $g$ changes from concave down to concave up at $x$ increases.  How does the second derivative of $g$ change as $x$ increases?
	
	
	
	\item $f'(x)<0$ on $(\-infty, -1)$; $f'(x)>0$ on $(-1,2)$; $f'(x)<0$ on $(5,\infty)$.
