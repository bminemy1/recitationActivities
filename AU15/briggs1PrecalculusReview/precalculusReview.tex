...
Try to draw a graph of a function $f$ which is always increasing, but $f^{-1}$ is not always increasing?

			 \begin{freeResponse}
			 
			This cannot be done, if $f$ is always increasing, then $f^{-1}$ is always increasing.  To prove this, suppose $x < y$.  Then it follows from the definition of $f^{-1}$ that $f(f^{-1}(x)) = x < y =  f(f^{-1}(y))$.  But since $f$ is increasing, this implies that $f^{-1}(x)  < f^{-1}(y)$.
